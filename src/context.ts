...
interface ContextTracker {
  /** Context created */
  createCtx?: (this: null, operation: string, isRootContext: boolean) => void
  /** Context finished */
  finishCtx?: (this: null) => void
  /** Exception logged */
  exception?: (this: null, err: Error) => void
  /** Authorization provided */
  setAuth?: (this: null, auth: Auth) => void
  /** Tag is set */
  setTag?: (this: null, name: string, value: TagValue) => void
}
...
export class Context {
  ...
  static #triggerTrackers<M extends keyof ContextTracker> (method: M, args: Parameters<NonNullable<ContextTracker[M]>>): void {
    for (const t of this.#contextTrackers) {
      const m = t[method]
      if (m == null) {
        continue
      }
      ;(m as Function).apply(null, args)
    }
  }
  ...
  constructor (operation: string, opts: CreateContextOpts = {}) {
    ...
    Context.#triggerTrackers('createCtx', [operation, opts.requestId == null])
  }
  ...
  setTag (name: string, value: TagValue): void {
    ...
    Context.#triggerTrackers('setTag', [name, value])
  }
  ...
  logError (msg: string, error?: any): void {
    ...
    if (error != null && error instanceof Error) {
      Context.#triggerTrackers('exception', [error])
    }
  }
  ...
  finish (): void {
    ...
    Context.#triggerTrackers('finishCtx', [])
  ...
  setAuth (auth: Auth | undefined): void {
    ...
    if (auth != null) {
      Context.#triggerTrackers('setAuth', [auth])
    }
  }
}
